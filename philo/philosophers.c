        ...
        if (i % 2 == 0)
            if(i == 2)
                if (cnt % 4 == 0 && cnt % 100 != 0)
                    month_sec[i] = 29 * DAY_SEC;
                else
                    month_sec[i] = 28 * DAY_SEC;
            else if(i < 7)
                    month_sec[i] = 30 * DAY_SEC;
            else
                month_sec[i] = 31 * DAY_SEC;
        else
            if(i < 7)
                month_sec[i] = 31 * DAY_SEC;
            else
...
int month_calculator(time_t now_time, int year)
{
    int now_month;
    int i;
    long *month_sec;
    ...
    i = 0;
    month_sec = get_month_sec();

    while(year > 1970)
    {
        if (year % 4 == 0)
            now_time -= 366 * DAY_SEC;
        else
            now_time -= 365 * DAY_SEC;
        year--;
    }
    i = 0;
